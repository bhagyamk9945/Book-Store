...
const Settings = () => {
  ...
  const headers = {
    ...
    const fetchUserDetails = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/v1/get-user-information',{headers}); // Adjust API endpoint accordingly
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put('http://localhost:5000/api/v1/update-address',{address}, { headers }); // Adjust API endpoint accordingly
